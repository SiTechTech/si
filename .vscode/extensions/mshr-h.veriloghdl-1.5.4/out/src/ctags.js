...
class Symbol {
    ...
    static getSymbolKind(name) {
        switch (name) {
            ...
            case 'port':
                return vscode_1.SymbolKind.Boolean;
            case 'register':
                return vscode_1.SymbolKind.Variable;
            case 'task':
                return vscode_1.SymbolKind.Function;
            case 'block':
                return vscode_1.SymbolKind.Module;
            case 'assert':
                return vscode_1.SymbolKind.Variable; // No idea what to use
            case 'class':
                return vscode_1.SymbolKind.Class;
            case 'covergroup':
                return vscode_1.SymbolKind.Class; // No idea what to use
            case 'enum':
                return vscode_1.SymbolKind.Enum;
            case 'interface':
                return vscode_1.SymbolKind.Interface;
            case 'modport':
                return vscode_1.SymbolKind.Boolean; // same as ports
            case 'package':
                return vscode_1.SymbolKind.Package;
            case 'program':
                return vscode_1.SymbolKind.Module;
            case 'prototype':
                return vscode_1.SymbolKind.Function;
            ...
            case 'typedef':
                return vscode_1.SymbolKind.TypeParameter;
            default:
                return vscode_1.SymbolKind.Variable;
        }
    }
...
class Ctags {
    ...
    execCtags(filepath) {
        ...
        let ctags = (vscode_1.workspace.getConfiguration().get('verilog.ctags.path', 'none'));
        if (ctags != 'none') {
            ...
            if (this.isDirty) {
                ...
                while ((match = eRegex.exec(text))) {
                    if (match && typeof match[1] !== 'undefined') {
                        ...
                        let s = this.symbols.filter((i) => i.type === match[1] &&
                        ...
                        if (s.length > 0) {
                            ...
                            for (let i = 0; i < s.length; i++) {
                                max = s[i].startPosition.isAfter(max.startPosition)
                                    ? s[i]
                                    : max;
                            }
